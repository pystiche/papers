...
class AddNoiseChannels(nn.Module):
    r"""Adds a certain number of noise channels in the size of the input to the input.
    Args:
...
class HourGlassBlock(SequentialWithOutChannels):
    r"""A special block of :cite:`ULVL2016` with a downsample, a middle component and an upsample of an input.
    Args:
        intermediate: Middle component of the block.
    """
...
class ConvSequence(SequentialWithOutChannels):
    r""" Defines a sequence of ConvBlocks that occurs repeatedly in :cite:`ULVL2016`.
...
class JoinBlock(nn.Module):
    ...
    Args:
        ...
        instance_norm: If ``True``, use :class:`~torch.nn.InstanceNorm2d` rather than
            :class:`~torch.nn.BatchNorm2d` as described in the paper. Additionally this flag is used for
            switching between the github branches. For details see FIXME.
        channel_dim: The dimension over which the tensors are concatenated. Defaults to 1.
    """
...
class BranchBlock(nn.Module):
    r"""Defines a block to merge the input from the current branch and the branch one step further in the Transformer
    from :cite:`ULVL2016`.
...
def level(
    ...
    Args:
        prev_level_block: Input from the previous level.
        impl_params: If ``True``, use the parameters used in the reference
            implementation of the original authors rather than what is described in
            the paper. For details see FIXME.
        instance_norm: If ``True``, use :class:`~torch.nn.InstanceNorm2d` rather than
...
class Transformer(nn.Sequential):
    ...
    Args:
        levels: Number of levels in the Transformer.
        impl_params: If ``True``, use the parameters used in the reference
            implementation of the original authors rather than what is described in
            the paper. For details see FIXME.
        instance_norm: If ``True``, use :class:`~torch.nn.InstanceNorm2d` rather than
            :class:`~torch.nn.BatchNorm2d` as described in the paper. Additionally this flag is used for
            switching between the github branches. For details see FIXME.
        init_weights: FIXME this should be removed.
    """
...
def transformer(
    ...
    Args:
        ...
        mpl_params: If ``True``, use the parameters used in the reference
            implementation of the original authors rather than what is described in
            the paper. For details see FIXME.
        instance_norm: If ``True``, use :class:`~torch.nn.InstanceNorm2d` rather than
            :class:`~torch.nn.BatchNorm2d` as described in the paper. Additionally this flag is used for
